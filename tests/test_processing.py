...
class TestFilterByState:
    ...
    @pytest.mark.parametrize(
        "state, expected_count", [("EXECUTED", 3), ("CANCELED", 2)]
    )
    def test_filter_by_state(
        self, operations_list: list, state: str, expected_count: int
    ) -> None:
    ...
    def test_sort_by_date_same_dates(self, operations_with_same_date: list) -> None:
        ...
        for op in result:
            ...
            if isinstance(date_str, str):
                assert date_str.startswith("2024-03-01")
        dates = [op.get("date", "") for op in result if isinstance(op.get("date"), str)]
        if dates:
            assert dates == sorted(dates, reverse=True)
